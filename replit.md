...
- **Per-user GitHub authentication**: Each user connects their own GitHub account via OAuth
- **GitHub starring integration**: Save button stars repos on the authenticated user's GitHub account
- **Template generation**: Users can convert repos to Replit-ready templates in their own GitHub account
...
- **Authentication System**:
  - Replit Auth (OpenID Connect) for user login with PostgreSQL session storage
  - Per-user GitHub OAuth for repository access (stored in `githubTokens` table)
  - Protected API endpoints requiring authentication and GitHub connection
...
- **Data Persistence**: Uses `localStorage` for client-side saving/skipping, with saved repos starred on the authenticated user's GitHub account via API.
...
- **Template Generation System**: Converts any GitHub repository into a Replit-ready template using the authenticated user's GitHub account. This involves cloning the repository, language/framework detection, cleaning unnecessary files, creating a `.replit` config, generating a new README, initializing a new Git repository, pushing to the user's GitHub, and launching in Replit.
...
- **Replit Auth (OpenID Connect)**: For user login and session management with PostgreSQL storage
- **GitHub OAuth App**: Per-user GitHub authentication for repository access and template creation
...
- **PostgreSQL**: Database for storing users, sessions, and GitHub OAuth tokens
